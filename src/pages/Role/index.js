...
import { getRoleList, getMenuList, getRoleDetail, updateRole, addRole, deleteRole } from '@/api'
// 获取查询参数
...
const Role = () => {
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="middle">
          ...
          <Button type="primary" onClick={() => deleteRoleInfo(record.id)} danger>
            删除
          </Button>
        </Space>
      ),
  ...
  const fetchData = async () => {
    setLoading(true)
    getRoleList(qs.stringify(queryParameters))
      .then(res => {
        // 处理数据
        setData(res.records)
        // 设置分页
  ...
  const getRoleInfo = async (id) => {
    ...
    if (id === 0) {
      ...
      getRoleDetail(id).then(res => {
        ...
        // 设置展开的树节点
        setCheckedKeys(res.controlIdList)
        // 打开抽屉
        showDrawer()
      }).catch(err => {
        console.log(err)
  ...
  const onCheck = (checkedKeysValue) => {
    ...
    // 同时设置表单数据
    form.setFieldsValue({
      controlIdList: checkedKeysValue,
    })
  }
  // 提交表单
    ...
    <>
      ...
      >
        ...
        >
          ...
          <Form.Item name="controlIdList" label="权限">
            <Tree
              checkable
              onExpand={onExpand}
